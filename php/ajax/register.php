            ...
            if ($email === $this->email){
                ...
                if($response['auth'])
                {
                    ...
                    if ($result instanceof db_error)
                        $this->json_error('Impossibilie salvare i dati nel database');
                    else 
                    {
                        if(!send_email($this->email, 'Asso Antincendio', 'Registrazione Asso Antincendio',
                ...
                if ($response['auth']) {
                    ...
                    if (send_email($this->email, 'Asso Antincendio', "Registrazione Asso Antincendio",
                       ...
                       if(!send_email(MAIL_AMMINISTRAZIONE, "Asso Antincendio", "Richiesta conferma registrazione",
                                      "Il cliente ". $this->name . " " . $this->surname . " con telefono " . $this->phone . " e email " . $this->email . " associata all'account <b style='color: #007139;'>" . $array_file['Anagrafica']['RAGIONE_SOCIALE'] . "</b> sta cercando di registrarsi al sito.<br><br>Cliccare su uno dei seguenti pulsanti per confermare o negare 
                                              la registrazione. <br><br><a href='" . LINK_APPLICATION . "php/ajax/confirm_registration.php?piva=" . $this->username . "&email=" . $this->email . "&name=" . $nome . "&password=" . $pass . "&new=1'><button style='background: #007139; color: #ffffff; border: none; -webkit-border-radius: 5px;-moz-border-radius: 5px;border-radius: 5px; padding: 10px;'>CONFERMA REGISTRAZIONE</button></a>
                                              <a href='" . LINK_APPLICATION . "php/ajax/deny_registration.php?piva=" . $this->username . "&email=" . $this->email . "&name=" . $nome . "'><button style='background: #E52612; color: #ffffff; border: none; -webkit-border-radius: 5px;-moz-border-radius: 5px;border-radius: 5px; padding: 10px;'>NEGA REGISTRAZIONE</button></a>"))
                                              $this->json_error("Impossibile inviare email di conferma, contattare l'assistenza per avere ulteriori informazioni!");
                     }
                    else $this->json_error("Impossibile inviare email di conferma, contattare l'assistenza per avere ulteriori informazioni!");
                }
                else{
                    $this->json_error("Questa email e' stata gia' respinta una volta dall'amministratore. Contattare l'assistenza per ulteriori informazioni");
                }
            }
        } else $this->json_error("Partita iva non trovata");
    }
