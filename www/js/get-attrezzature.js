                    ...
                    $.each(value, function (innerKey, innerValue) {
                        ...
                        if(innerKey === 'contratto') {
                            ...
                            $.each(innerValue, function (lastKey, lastValue) {
                                ...
                                let list;
                                if (name === 'RILEVATORI FUMI'){
                                    list = $('<a href="#' + name.toLowerCase() + '" class="ui-btn margin-top-12 box-shadow-bottom border-radius-10 red-text" data-name="' + contratto + '">IMPIANTO RIVELAZIONE INCENDI</li>').on('click', function () {
                                        viewList(lastValue, $(this).attr('data-name'));
                                        $.mobile.changePage('#viewList');

                                    });
                                }else {
                                    list = $('<a href="#' + name.toLowerCase() + '" class="ui-btn margin-top-12 box-shadow-bottom border-radius-10 red-text" data-name="' + contratto + '">' + name + '</li>').on('click', function () {
                                        viewList(lastValue, $(this).attr('data-name'));
                                        $.mobile.changePage('#viewList');
