            ...
            if (data.result) {
                ...
                $.each(data[0], function (key, value) {
                    ...
                    if(key === 'RAGIONE_SOCIALE' || key === 'INDIRIZZO_FATTURAZIONE' || key === 'INDIRIZZO_SPEDIZIONE'
                        ...
                        || key === 'CELLULARE') {
                        let field = $('<label for="' + key + '" class="center-text bold-text border-bottom-1-gray font-large box-shadow-bottom">' +  key.replace("_", " ") + '</label><input type="text" name="' + key + '" id="' + key + '" placeholder="' + value + '"><')
    ...
    $('#inviaCambioAnagraficaDati').on('click', function () {
        ...
        $('#cambioAnagraficaForm').children().each(function (key, value) {
            ...
            if (elem.prop('tagName') === 'DIV') {
                ...
                console.log(elem.find('input').attr('placeholder'));
        ...
        for (var val of emailForm.values()){
            console.log(val);
        }
